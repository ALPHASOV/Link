...
#define SFR_COND_4(Condition) (((Condition) && (Condition)) && ((Condition) && (Condition)))

// GET_CYCLES: Converts Time[us] and MaxSysFreq[Hz] into cycles for driver timer,
// rounding up; for compile-time use only.
#define GET_CYCLES(Time, MaxSysFreq)	((uword)(((((long)(Time))*((MaxSysFreq)/1000))+7999)/8000))
...
#define SCS_MASK_KSCCFG_MODEN	1U   // MODEN Module Enable
#define SCS_MASK_KSCCFG_BPMODEN	(1U << 1U)   // BPMODEN Bit Protection for MODEN, set to 1 for change (w)
#define SCS_MASK_KSCCFG_BPNOM	(1U << 7U)   // BPNOM Bit Protection for NOMCFG, set to 1 for change (w)
#define SCS_MASK_KSCCFG_BPSUM	(1U << 11U)  // BPSUM Bit Protection for SUMCFG, set to 1 for change (w)
#define SCS_MASK_KSCCFG_BPCOM	(1U << 15U)  // BPCOM Bit Protection for COMCFG, set to 1 for change (w)

// Constants for PLLCON0 register
#define SCS_MASK_PLLCON0_NDIV	(0x3FU << 8U)   // NDIV N-Divider Value 16...40 for VCOSEL = 1
#define SCS_MASK_PLLCON0_NACK	(1U << 15U)   // N-Divider Ready Acknowledge
#define SCS_POS_PLLCON0_NDIV 8U

// Constants for PLLCON1 register
#define SCS_MASK_PLLCON1_OSCSEL	(1U << 1U)   // OSCSEL Oscillator Input Selection
#define SCS_MASK_PLLCON1_AOSCSEL	(1U << 3U)   // AOSCSEL Asynchronous Oscillator Input Selection
#define SCS_MASK_PLLCON1_PDIV	(0x0FU << 8U)   // PDIV P-Divider Value 0...15
#define SCS_MASK_PLLCON1_PACK	(1U << 15U)  // P-Divider Ready Acknowledge
...
#define SCS_MASK_PLLSTAT_VCOBYST	(1U << 0U)   // VCOBYST PLL clock derived from divider K2 (rh)
#define SCS_MASK_PLLSTAT_OSCSELST	(1U << 2U)   // OSCSELST Internal clock for PLL (rh)
...
typedef struct
{
  uword K2;      // K2 step value
  uword Delay;   // delay in timer cycles after applying K2 step
} Scs_K2RampType;
...
static const Scs_K2RampType Scs_K2RampNormalUp[SCS_K2_RAMP_STEPS] =
{
  ...
  // Saved CCU60 SFRs
  static uword Ccu60KscfgSave;
  static uword Ccu60Tctr0Save;
  static uword Ccu60Tctr2Save;
  static uword Ccu60PiselhSave;
  static uword Ccu60T13prSave;
...
// INLINE functions
INLINE Scs_ErrorType Scs_lEnableVcoBypass(uword Enable);
INLINE Scs_ErrorType Scs_lSelectExternalPllClock(void);
INLINE void Scs_lStartTimer(uword Cycles);
INLINE Scs_ErrorType Scs_lWaitForVcoLock(uword Timeout);

// Functions with standard location
static Scs_ErrorType Scs_lApplyNewK1Div(uword K1Div);
static Scs_ErrorType Scs_lApplyNewNDiv(uword NDiv);
static Scs_ErrorType Scs_lApplyNewPDiv(uword PDiv);
static Scs_ErrorType Scs_lApplyNewVcoDivs(uword PDiv, uword NDiv,uword K2Div);
...
//*****************************************************************************
// @Function      Scs_ErrorType Scs_lWaitForVcoLock(uword Timeout) 
//
//-----------------------------------------------------------------------------
...
INLINE Scs_ErrorType Scs_lWaitForVcoLock(uword Timeout)
{
  ...
  } while((!(SCU_PLLSTAT & SCS_MASK_PLLSTAT_VCOLOCK)) && SCS_T13_RUN);
  if(!(SCU_PLLSTAT & SCS_MASK_PLLSTAT_VCOLOCK))
  {
	  return SCS_ERROR_TO_VCO_LOCK;
  }
  // No error
...
//*****************************************************************************
// @Function      Scs_ErrorType Scs_lEnableVcoBypass(uword Enable) 
//
//-----------------------------------------------------------------------------
...
//*****************************************************************************
// @Function      INLINE void Scs_lStartTimer(uword Cycles)
//
//-----------------------------------------------------------------------------
...
//*****************************************************************************
// @Function      Scs_ErrorType Scs_lApplyNewK1Div(uword K1Div)
//
//-----------------------------------------------------------------------------
...
//*****************************************************************************
// @Function      Scs_ErrorType Scs_lApplyNewNDiv(uword NDiv) 
//
//-----------------------------------------------------------------------------
...
//*****************************************************************************
// @Function      Scs_ErrorType Scs_lApplyNewPDiv(uword PDiv) 
//
//-----------------------------------------------------------------------------
...
//*****************************************************************************
// @Function      Scs_ErrorType Scs_lApplyNewVcoDivs(uword PDiv,uword NDiv, uword K2Div) 
//                
//
//-----------------------------------------------------------------------------
...
static Scs_ErrorType Scs_lCheckFreqHighPrecOsc(void)
{
  ...
  // Initialize attempts
  Attempts = (uword)SCS_ATTEMPTS_OSC_HP;
  ...
  do
  {
	...
	} while((!SCU_HPOSCCON_PLLV) && SCS_T13_RUN);
	if(!SCU_HPOSCCON_PLLV) 
	{
		return SCS_ERROR_TO_OSC_HP_PLLV;
	}
	...
	} while(((!SCU_HPOSCCON_OSC2L1) && (!(SCU_PLLSTAT & SCS_MASK_PLLSTAT_OSCLOCK))) && SCS_T13_RUN);
...
static Scs_ErrorType Scs_lEnableVcoLockEmerg(void)
{
  ...
  if((Work & (SCS_MASK_PLLSTAT_VCOL0 | SCS_MASK_PLLSTAT_VCOL1)) || (!(Work & SCS_MASK_PLLSTAT_VCOLOCK)))
  {
    // error: VCO unlocked
    return SCS_ERROR_VCO_UNLOCKED;
  }
...
//*****************************************************************************
// @Function      Scs_ErrorType Scs_ApplyNewK2Div(uword K2Div)
//
//-----------------------------------------------------------------------------
...
Scs_ErrorType Scs_ApplyNewK2Div(uword K2Div)
{
...
Scs_ErrorType Scs_GoFromBaseModeToNormalMode(void)
{
  ...
  Error = Scs_lApplyNewVcoDivs((((uword)SCS_P)) - 1U, (((uword)SCS_N)) - 1U, (((uword)SCS_K2_10MHZ)) - 1U);
  if(Error)
  {
    return Error;
  }
  ...
  Error = Scs_lWaitForVcoLock(GET_CYCLES(SCS_TIME_VCO_LOCK,SCS_F_INT_10MHZ_MAX));
  if(Error)
  {
    return Error;
  }
...
// USER CODE END
  ...
  void Scs_RestoreTimer(void)
  {
    ...
    CCU60_KSCFG = Ccu60KscfgSave | SCS_MASK_KSCCFG_BPMODEN | SCS_MASK_KSCCFG_BPNOM | SCS_MASK_KSCCFG_BPSUM | SCS_MASK_KSCCFG_BPCOM;
